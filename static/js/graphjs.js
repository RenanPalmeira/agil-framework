...
GraphJs.prototype = {
	...
	nodePos: function(node){
		...
		console.log(top);
		console.log(this.canvas.offsetHeight-this.canvas.offsetTop);
		console.log(left);
		console.log(this.canvas.offsetLeft);
		var edge = {
			'x':{
				'min': ((left+node.x)-node.radius),
				'max': ((left+node.x)+node.radius)
			},
			'y':{
				'min': ((top+node.y)-node.radius),
				'max': ((top+node.y)+node.radius)
			}
		};
		return edge;
	},
	...
	relationship: function (startNode, endNode) {
		if(startNode == null){
	...
	eventing: function(event, node, callback) {
		var self = this;
		var renan = this.nodePos(node);
		var lisp_callback = function(e) {
			var mouse = self._getMousePos(self.canvas, e);
			var y = mouse.y;
			var x = mouse.x;
			console.log(y);
			if(x>=renan.x.min && x<=renan.x.max && y>=renan.y.min && y<=renan.y.max) {
				console.log("fsd");
			}
		...
		this.canvas.addEventListener("mousemove", lisp_callback);
	}
};