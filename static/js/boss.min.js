...
var boss = {
	...
	modalWindow: function(datas){
		var fields_single = ['pk', 'pk2', 'title', 'content'];
		var fields_multiple = ['members'];

		$('#modal_window').addClass('active');

		if(datas['members'] == fields_multiple[0]){
			$("#window_members").empty();
			$("#window_members").append("<b>Membros:</b>");
			$.each(datas['members'], function(index, value) {
				$("#window_members").append("<p class='badge' style='margin-left:2px;'>"+value+"</p>");
			});
		}
		
		//if(datas['title'] == fields_single[2]){
			$("#window_title").empty();
			$("#window_title").append("<p>"+datas['title']+"</p>");
		//}
		//if(datas['content'] == fields_single[3]){
		...
		$("#window_check").on('click', function(e){
			...
			//boss.ajax.load('/app/task/form_delte/?pk='+datas['pk'], '#modal_dialog', 'active');
		});
