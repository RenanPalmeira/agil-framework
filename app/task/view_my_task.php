					...
					<div class="card-content">
						<div class="col-12">
							...
							<div class="col-3 pull-left text-right">
								<div class="btn-group">
									<button class="btn btn-primary" onclick="boss.ajax.load('/app/task/view_my_task/?pk=<?php echo $pk; ?>', '#app_pane_body');">Abertas</button>
									<button class="btn" onclick="boss.ajax.load('/app/task/view_my_close_task/?pk=<?php echo $pk; ?>', '#app_pane_body');">Entregues</button>
								</div>
							</div>
						</div>
					</div>
						...
						if(count($tasks)>0){
							...
							if($rsItem['status']==1){
								if($rsItem['id_project_task']==$task['id_project_task'] || $logado['id_member']==$rsItemMember['id_admin']){
									?>
									<div class="card card-effect" onclick="boss.ajax.load('/app/task/view_item/?pk=<?php echo $rsItem['id_project_task_items']; ?>;', '#modal_window', 'active');">
										<div class="card-content">
												...
												<?php 
													...
													foreach ($rsMembers as $rsMember) {
														...
														<div class="col-6 pull-left">
															...
															<span class="btn btn-xs"><?php echo $rsMember['name']; ?></span>
														</div>
														<?php
													}
